...
namespace :pipeline do
  namespace :dev do
    ...
    task push_to_dev: :environment do
      ...
      resp = Net::HTTP.get_response URI("https://api.github.com/repos/govuk-forms/forms-product-page/commits/#{git_hash}")
      if resp.code != "200"
...
def push_commit_to_dev(git_hash)
  json_input = {
    name: "forms-product-page-image-builder",
    sourceRevisions: [
      {
  ...
  puts ""
  puts "Starting pipeline 'forms-product-page-image-builder' in the 'deploy' account"
  pp json_input
